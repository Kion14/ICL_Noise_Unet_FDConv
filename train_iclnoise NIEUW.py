...
from DataAugmentation import random_intensity_augmentation, random_invert_intensity, degrade_target_image
import random
import json
...
EXPERIMENT_NAME = "2juni_GOODCTX_9eRUN_ICL_NMB_ctx4"
...
class TrainDataset(Dataset):
    ...
    def __getitem__(self, idx):
        if self.return_context_only:
            ...
            target_img, target_mask, target_stain, target_sample_id = self.data[idx]
            ...
            target_img = degrade_target_image(target_img, p=0.5)
    ...
    #        return img, mask
            ...
            same_stain_indices = [
                i for i, item in enumerate(self.context_dataset)
                if item[2] == target_stain and item[3] != target_sample_id
            ]

            candidate_indices = same_stain_indices
            if len(candidate_indices) < k:
                candidate_indices = [
                    i for i, item in enumerate(self.context_dataset)
                    if item[3] != target_sample_id
                ]

            # mask filtering
            filtered_indices = []
            for i in candidate_indices:
                _, ctx_mask, *_ = self.context_dataset[i]
                ratio = ctx_mask.mean()
                if 0.01 < ratio < 0.70:
            ...
            # retrieval op basis van grayscale L2
            target_tensor_tmp = torch.tensor(
                np.ascontiguousarray(target_img),
                dtype=torch.float32
            ).permute(2, 0, 1)

            target_gray = target_tensor_tmp.mean(dim=0)
            target_gray = (target_gray - target_gray.mean()) / (target_gray.std() + 1e-6)

            distances = []
            for i in candidate_indices:
                ctx_img, _, *_ = self.context_dataset[i]

                ctx_tensor = torch.tensor(
                    np.ascontiguousarray(ctx_img),
                    dtype=torch.float32
                ).permute(2, 0, 1)
                ...
                dist = torch.norm(target_gray - ctx_gray).item()
                distances.append((dist, i))

            distances.sort(key=lambda x: x[0])
            context_indices = [i for _, i in distances[:k]]

            # fallback als te weinig context
            while len(context_indices) < k:
                context_indices.append(random.choice(candidate_indices))
